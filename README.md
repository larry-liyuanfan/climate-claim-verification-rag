...
| Reranking | Configurable 0.6B/4B local Qwen3 model, Alibaba Model Studio adapter, deterministic feature fallback | 0.6B exposed first-stage replacement failure; 4B plus balanced rank fusion improved all four fixed-dev ranking metrics with paired intervals above zero, and remains an offline quality profile |
