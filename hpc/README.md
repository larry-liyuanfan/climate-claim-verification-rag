...
`rebuild_candidate_supported_ltr.sbatch` is the exact-SHA replacement gate. It derives its 30-minute/24-GB shape from the earlier 10:11, 12.1-GB feature job, rebuilds candidate-supported rows, trains LambdaMART and evaluates the same 154-claim dev split in one serial allocation. The current version uses 100 hard negatives per train claim to match the 100-candidate serving fusion width, records the RRF score/rank prior in both training and inference, and reports a predeclared 4:1 RRF/LambdaMART rank fusion. Its deterministic reranker stage remains a fallback diagnostic and must not be described as Qwen3.
