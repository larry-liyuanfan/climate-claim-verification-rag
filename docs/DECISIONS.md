...
The fixed-dev run first selected BM25+dense RRF over BM25. A pure Qwen3-Reranker-0.6B replacement then regressed because it discarded the strong RRF order. The corrected experiment preserves that order through weighted rank fusion: the 4:1 RRF/Qwen profile improved Recall@5, MRR@10 and nDCG@10 with paired intervals above zero, while its Evidence F1 interval still crossed zero. Because Qwen adds P50/P95 `5.57/6.52 s` per query, HNSW+RRF remains the latency-oriented default and the fused system is an offline quality profile. The trained LambdaMART and deterministic reranker still regress sharply. IVF-PQ remains rejected because Recall@5 versus Flat was only `0.3688`; HNSW remains the ANN default.
