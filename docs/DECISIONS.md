...
The fixed-dev run first selected BM25+dense RRF over BM25. A pure Qwen3-Reranker-0.6B replacement then regressed because it discarded the strong RRF order. Weighted rank fusion corrected that architectural error. The next size gate compared 4B on the identical split and resource-tested it with BF16 on a 20 GB MIG slice. A balanced RRF/4B fusion improved Recall@5, MRR@10, nDCG@10 and Evidence F1 with paired intervals above zero. It is the offline quality profile, while HNSW+RRF remains the latency default because even the 4B run adds P50/P95 `4.20/4.82 s` per query. Model size and fusion weights were selected on the same dev split, so the result is not described as independent test generalisation. The trained LambdaMART and deterministic reranker still regress sharply. IVF-PQ remains rejected because Recall@5 versus Flat was only `0.3688`; HNSW remains the ANN default.
